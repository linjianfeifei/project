...
use think\Response;
use think\Session;
use app\index\model\Professor as ProfessorModel;
...
class Professor extends Base
{
    ...
    public function index()
    {
        ...
        $this->view->assign('title', '教授用户列表');
        $this->view->assign('keywords', '');
        $this->view->assign('desc', '');

        $this->view->count = ProfessorModel::count();

        $list = ProfessorModel::paginate(6); //每页显示6条
    ...
    public function adminEdit(Request $request)
    {
        ...
        $result = ProfessorModel::get($user_id);
        $this->view->assign('title','编辑教授用户信息');
        $this->view->assign('keywords','');
        $this->view->assign('desc','');
...
//        $data = $request -> param();
        ...
        $result = ProfessorModel::update($data, $condition);
    ...
    {
        $user_id = $request->param('id');
        ProfessorModel::update(['is_delete' => 1], ['id' => $user_id]);
        ProfessorModel::destroy($user_id);
    ...
    {
        ProfessorModel::update(['delete_time' => NULL], ['is_delete' => 1]);
    }

    //添加操作的界面
    public function  adminAdd()
    {
        $this->view->assign('title','添加教授用户');
        $this->view->assign('keywords','');
        $this->view->assign('desc','');
